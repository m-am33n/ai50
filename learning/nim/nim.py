...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        `state`, return 0.
        """
        if not self.available_actions(state):
            return 0
        max_reward = -math.inf
        for action in self.available_actions(state):
            max_reward = max(max_reward, self.get_q_value(state, action))
        return max_reward

    def available_actions(self, state):
        return Nim.available_actions(state)
    def choose_action(self, state, epsilon=True):
        ...
        """
        prob = random.random()
        if epsilon and prob < self.epsilon:
